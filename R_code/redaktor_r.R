data=read.csv("C:\\Users\\Mrinal\\Desktop\\minor porject\\data\\redaktor.csv")
data=data[,-22]
View(data)
...
data_redaktor=cbind(test_data,test.pred_lr,test.pred_rf,test.pred_svm)
View(data_redaktor)
write.csv(data_redaktor,"data_redaktor.csv")

library(VennDiagram)
# Taking a subset of the data of predicted labels
tempdf = data_ant[,c(1,22,23,24)]
...
tempdf_deffective=tempdf[which(tempdf$target==1),]
# This code generates black and white venn diagram
# NOTE - Change the file location to get the output. 
venn.diagram(x = list(pred_lr = tempdf_deffective[, "test.pred_lr_new"], pred_rf = tempdf_deffective[,"test.pred_rf_new"],pred_svm = tempdf_deffective[, "test.pred_svm_new"]), filename = "redaktor_b&w_deffective.png")

# This plots a colorful venn diagram, however the labels are not placed
# in correct location.
venn.diagram(x = list(pred_lr = tempdf_deffective[, "test.pred_lr_new"],pred_rf = tempdf_deffective[, "test.pred_rf_new"],pred_svm = tempdf_deffective[, "test.pred_svm_new"]), filename = "redaktor_coloured_deffective.png",col = "transparent", fill = c("cornflowerblue","green","yellow"),alpha = 0.50, label.col = c("orange", "white", "darkorchid4", "white", "white","darkgreen", "white"),cex = 1.5, fontfamily = "serif", fontface = "bold",cat.col = c("darkblue", "darkgreen", "orange"), cat.cex = 1.5,cat.pos = 0, cat.dist = 0.07, cat.fontfamily = "serif", rotation.degree = 270,margin = 0.2)

tempdf_correct=tempdf[which(tempdf$target==0),]
# This code generates black and white venn diagram
# NOTE - Change the file location to get the output. 
venn.diagram(x = list(pred_lr = tempdf_correct[, "test.pred_lr_new"], pred_rf = tempdf_correct[,"test.pred_rf_new"],pred_svm = tempdf_correct[, "test.pred_svm_new"]), filename = "redaktor_b&w_correct.png")

# This plots a colorful venn diagram, however the labels are not placed
# in correct location.
venn.diagram(x = list(pred_lr = tempdf_correct[, "test.pred_lr_new"],pred_rf = tempdf_correct[, "test.pred_rf_new"],pred_svm = tempdf_correct[, "test.pred_svm_new"]), filename = "redaktor_coloured_correct.png",col = "transparent", fill = c("cornflowerblue","green","yellow"),alpha = 0.50, label.col = c("orange", "white", "darkorchid4", "white", "white","darkgreen", "white"),cex = 1.5, fontfamily = "serif", fontface = "bold",cat.col = c("darkblue", "darkgreen", "orange"), cat.cex = 1.5,cat.pos = 0, cat.dist = 0.07, cat.fontfamily = "serif", rotation.degree = 270,margin = 0.2)
